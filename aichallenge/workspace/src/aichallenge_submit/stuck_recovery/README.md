...
| その状態が `stuck_duration` 継続 | **1.5 s** | 検知までの時間は丸ごと「障害物に全開で押し付けている時間」。誤検知は起きない — 1.5 s で 1.0 m 以内に留まるには 0.67 m/s 未満である必要がある |
...
### 復帰サイクルは短い方が速い (2026-08-15, dashboard R69)

1 回のスタックにかかる時間は `stuck_duration + シフト + 後退 + シフト + 前進 + cooldown`
で約 11 秒あり、詰まりが 20 回起きれば 200 秒を超える。**R68 で「遅くしてペナルティを
減らす」方向が負けている**（4周 / 5周）ので、逆に `stuck_duration` 2.5→1.5 s、
`cooldown` 3.0→1.5 s と詰めた。サイクルは約 8.9 秒になる。

再試行が 12.2 秒のペナルティ窓により深く入るのでペナルティは増える側に振れるが、
それが正しいトレードだというのが R68 の結論である。結果は **6/6 完走・480 秒**、
リカバリ 16 回、ペナルティ 134.0 秒。詰まりを抜けた後の4周は
**49.8 / 49.8 / 49.7 / 49.6 秒**で、ハンディキャップ下のクリアラップそのもの。
